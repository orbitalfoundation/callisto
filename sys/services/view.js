...
class ViewSingleton {
	...
	constructor() {
		...
		let scene1 = this.scene1 = new BABYLON.Scene(this.engine)
		let scene2 = this.scene2 = new BABYLON.Scene(this.engine) ; scene2.autoClear = false;

		if(this.PHYSICS) {
			let physicsPlugin = new BABYLON.CannonJSPlugin()
			this.scene1.enablePhysics(new BABYLON.Vector3(0,-9.81, 0), physicsPlugin)
		}

		// scene 2 camera
		if(scene2) {
			let camera2 = this.camera2 = new BABYLON.UniversalCamera("UniversalCamera", new BABYLON.Vector3(0, 10, 10), this.scene2 )
			camera2.setTarget(BABYLON.Vector3.Zero())

			let light = new BABYLON.HemisphericLight("light",new BABYLON.Vector3(0, 0, 0),this.scene2)

		}

		//const camera1 = new BABYLON.FreeCamera("FreeCamera", new BABYLON.Vector3(0, 1, -15), scene1);

		//let camera1 = this.camera1 = new BABYLON.ArcRotateCamera("camera", -Math.PI / 2, Math.PI / 2.5, 3, new BABYLON.Vector3(0, 0, 0))
		let camera1 = this.camera1 = new BABYLON.UniversalCamera("UniversalCamera", new BABYLON.Vector3(0, 10, 10), this.scene1)
		camera1.setTarget(BABYLON.Vector3.Zero())
		camera1.lowerRadiusLimit = 2
		camera1.upperRadiusLimit = 20
		camera1.attachControl(this.canvas, true)
		camera1.inputs.addMouseWheel()
		camera1.inputs.attached["mousewheel"].wheelYMoveRelative = BABYLON.Coordinate.Y
		camera1.inputs.attached["mousewheel"].wheelPrecisionY = -1
		this.canvas.focus()
	...
	babylon_material(fragment) {

		// make a material from args

		let mat = new BABYLON.StandardMaterial("material",this.scene1);
		...
		if(fragment.art) {
			mat.diffuseTexture = new BABYLON.Texture(fragment.art, this.scene1)
		...
		var texture = new BABYLON.DynamicTexture("dynamic texture", {width:512, height:256}, this.scene1)
	...
	create_ux(fragment) {
		...
		var manager = new BABYLON.GUI.GUI3DManager(this.scene1);
...
//near.orientToCameraDeadzoneDegrees = 0;
		...
		return new BABYLON.TransformNode(uuid,this.scene1)
	}

	create_text(fragment) {
		let uuid = fragment.uuid
		let art = fragment.art
	...
	create_box(fragment) {
		...
		//mesh = BABYLON.Mesh.CreateGroundFromHeightMap(uuid,fragment.heightmap, 4, 4, 100, 0, 0.01, this.scene1, false)
		//mesh = BABYLON.Mesh.CreateGround(uuid, 1, 1, 10, this.scene1);
		if(fragment.walkable) {
			console.log("view: this object is a shadow receiver " + uuid)
			mesh = BABYLON.MeshBuilder.CreateGround("ground", {height: fragment.xyz[1], width: fragment.xyz[0], subdivisions: 8},this.scene1)
			mesh.receiveShadows = true

			mesh = BABYLON.Mesh.CreateGroundFromHeightMap("ground", "sys/apps/multiverse/textures/heightMap.png", 2, 2, 100, 0, 20, this.scene1, false);

			var groundMaterial = new BABYLON.StandardMaterial("ground", this.scene1);
			groundMaterial.diffuseTexture = new BABYLON.Texture("sys/apps/multiverse/textures/ground.jpg", this.scene1);
			...
			mesh = BABYLON.MeshBuilder.CreateBox(uuid,{},this.scene1)
		}

		return mesh
	}

	create_sphere(fragment) {
		return BABYLON.MeshBuilder.CreateSphere(fragment.uuid,{},this.scene1)
	}

	create_light(fragment) {
...
return 0;
			...
			let light = new BABYLON.HemisphericLight(fragment.uuid,new BABYLON.Vector3(0, 1, 0),this.scene1)
			return light
		} else {

			// make a point light
			let lightpos = new BABYLON.Vector3(...fragment.xyz);
			//let light = new BABYLON.PointLight(fragment.uuid, lightpos, this.scene1);
			var light = new BABYLON.DirectionalLight(fragment.uuid, new BABYLON.Vector3(-1, -2, -1), this.scene1);
//			light.position = lightpos
//			light.intensity = fragment.intensity || 2.5;

			// make art - todo make optional - or have as debug
			var lightSphere = BABYLON.Mesh.CreateSphere("sphere", 10, 0.1, this.scene1);
			lightSphere.position = light.position;
			lightSphere.material = new BABYLON.StandardMaterial("light", this.scene1);
			lightSphere.material.emissiveColor = new BABYLON.Color3(1, 1, 0);
//			light.falloffType = 2

			// make shadow support for first light - todo maybe formalize
			if(!this.shadowGenerator) {
				this.shadowGenerator = new BABYLON.ShadowGenerator(4096, light)
				console.log("view: created shadow emitter")
			...
			if(false || fragment.attach_to_camera) {
				...
				this.scene1.registerBeforeRender( () => {
	...
	debug(mesh) {
		...
		const box = BABYLON.Mesh.CreateBox("bounds",this.scene1)
		...
		box.edgesWidth = 1.0
		box.material = new BABYLON.StandardMaterial("material",this.scene1)
		box.material.alpha = 0.1
	}

	create_gltf(fragment) {
		let uuid = fragment.uuid
		let art = fragment.art
		let root = new BABYLON.TransformNode(uuid,this.scene1)

		if(fragment.debugbox) {
			root.material = this.babylon_material({alpha:0.3, art: fragment.debugbox })
		}

		let scene = fragment.scene ? this.scene2 : this.scene1
		...
		BABYLON.SceneLoader.ImportMesh(null,art,"scene.gltf", scene, (meshes,particles,skeletons) => {
		    ...
		    // insert gltf into root
		    scale.parent = root

			// add debug box around root - todo move
			if(fragment.debug) {
				this.debug(root)
			}

			// Enable shadows if in main scene only
			if(this.shadowGenerator && !fragment.scene) {
				let recurse = (m) => {
					if(m.isPickable) {
	...
	babylon_create_node(fragment) {
		let uuid = fragment.uuid
		let art = fragment.art
		switch(fragment.kind) {
			case "textarea": return this.create_text(fragment)
			case "ground":
			case "box": return this.create_box(fragment)
			case "sphere": return this.create_sphere(fragment)
			case "camera": return this.camera1
			case "light": return this.create_light(fragment)
			case "gltf": return this.create_gltf(fragment)
			case "glb": return this.create_gltf(fragment)
			default: return new BABYLON.TransformNode(uuid,this.scene1)
		}
	}
	...
	babylon_sdl(fragment,view) {
		...
		if(!node) {
			...
			node = this.babylon_nodes[uuid] = this.babylon_create_node(fragment)
			if(!node) {
		...
		if(fragment.material && !fragment.material_was_updated) {
			...
			node.material = this.babylon_material(fragment.material)
		}
	...
	//
		...
		if(fragment.lookat && !(node.lookat && node.lookat.x == fragment.lookat[0] && node.lookat.y == fragment.lookat[1] && node.lookat.z==fragment.lookat[2])) {
			...
			this.camera1.setTarget(new BABYLON.Vector3(...fragment.lookat))
		...
		if(this.PHYSICS) {
			...
			if(fragment.physics && !node.physics) {
				...
				node.physics = new BABYLON.PhysicsImpostor(node, BABYLON.PhysicsImpostor.BoxImpostor, { mass: mass, restitution: restitution }, this.scene1);
			}
		...
		// revise shadow?
		if(this.shadowGenerator && !fragment.shadow_was_updated) {
			fragment.shadow_was_updated = 1
			if(fragment.hasOwnProperty("shadow")) {
				this.shadowGenerator.addShadowCaster(node)
				console.log("...view: item casts a shadow " + node.uuid)
			} else {
//				this.shadowGenerator.removeShadowCaster(node)
//				console.log("...view: item has no shadow " + node.uuid)
	...
	register_handlers() {
		...
		window.addEventListener("click", (evt) => {
			...
			var pickResult = this.scene1.pick(evt.clientX, evt.clientY)
